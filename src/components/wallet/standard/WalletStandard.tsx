...
import {
  ...
  Fee,
  Form,
  TextField as ChinillaTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
...
import { /* chin_to_chinilla_string, */ chinilla_to_chin } from '../../../util/chinilla';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {chin_to_chinilla_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of chinilla in the blockchain at the current
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      openDialog(
        <AlertDialog>
          <Trans>
            Error: Cannot send chinilla to coloured address. Please enter a chinilla
            address.
          </Trans>
        </AlertDialog>,
      );
    ...
    const amountValue = Number.parseFloat(chinilla_to_chin(amount));
    const feeValue = Number.parseFloat(chinilla_to_chin(fee));
