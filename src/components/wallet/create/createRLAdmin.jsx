...
import { AlertDialog } from '@chinilla/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      chinillaper_input.value === '' ||
      Number(chinillaper_input.value) === 0 ||
      !Number(chinillaper_input.value) ||
      isNaN(Number(chinillaper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const chinillaper = chinilla_to_chin(chinillaper_input.value);
    const chinillaper_value = Number.parseInt(Number(chinillaper));
    const userpubkey = userpubkey_input.value;
    const amount = chinilla_to_chin(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = chinilla_to_chin(fee_input.value);
