...
import { AlertDialog, Card, Flex, Loading, Dropzone } from '@chinilla/core';
import {
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {chin_to_chinilla_string(props.balance)} TXCHI
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'chinilla_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'chinilla_addr://') {
