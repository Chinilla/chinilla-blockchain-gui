...
export default function Block() {
  ...
  const poolReward = chin_to_chinilla(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = chin_to_chinilla(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const chinillaFees = blockRecord.fees
    ? chin_to_chinilla(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.chinillaexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.chinillaexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: chinillaFees ? `${chinillaFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Chinilla blockchain
            </Trans>
          </Back>
        }
        action={
