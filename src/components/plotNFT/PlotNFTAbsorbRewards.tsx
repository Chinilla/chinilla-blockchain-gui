...
import { chin_to_chinilla, chinilla_to_chin } from '../../util/chinilla';
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const feeChins = chinilla_to_chin(fee);

      if (walletId === undefined || !address) {
        return;
      }

      await dispatch(pwAbsorbRewards(walletId, feeChins));
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat
                  value={chin_to_chinilla(BigInt(balance))}
                  display="inline"
                  state={State.SUCCESS}
                />{' '}
                to {wallet?.address}
