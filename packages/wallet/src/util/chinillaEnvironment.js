...
const PY_FOLDER = '../../../chinilla/daemon';
...
const getChinillaVersion = () => {
  let version = null;
  const exePath = getExecutablePath('chinilla');
  // first see if we can get a chinilla exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or chinilla is on the path
    try {
      version = child_process
