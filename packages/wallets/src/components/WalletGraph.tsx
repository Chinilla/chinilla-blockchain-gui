import { TransactionType, WalletType } from '@chinilla/api';
import type { Transaction } from '@chinilla/api';
import { useGetWalletBalanceQuery } from '@chinilla/api-react';
import { vojoToChinilla, vojoToCAT, blockHeightToTimestamp } from '@chinilla/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? vojoToCAT(start) : vojoToChinilla(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? vojoToCAT(balance) : vojoToChinilla(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? vojoToCAT(start) : vojoToChinilla(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? vojoToCAT(start) : vojoToChinilla(start).toString(), // bignumber is not supported by react
    });
  });
