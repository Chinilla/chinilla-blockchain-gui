  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, vojoToChinilla(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: vojoToChinilla(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, vojoToChinilla(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: vojoToChinilla(start).toString, // bignumber is not supported by react
    });
  });
