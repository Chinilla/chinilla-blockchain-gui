...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'xchi' ? Number(chinToChinilla(makerEntries[0][1])) : Number(chinToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'xchi' ? Number(chinToChinilla(takerEntries[0][1])) : Number(chinToCAT(takerEntries[0][1]));
