...
import { stopAllChinilla } from '../utils/wallet';
...
test('Confirm Enable Auto Login feature works as expected. ', async () => {
  ...
  await new LoginPage(page).login('password2022!@')
  await page.locator('[data-testid="DashboardSideBar-settings"]').click();
  await page.waitForURL('file:///Users/jahifaw/Documents/Code/chinilla-tn-pw-latest/chinilla-blockchain/chinilla-blockchain-gui/packages/gui/build/renderer/index.html#/dashboard/settings/general');
  await page.close();
  stopAllChinilla()
});
