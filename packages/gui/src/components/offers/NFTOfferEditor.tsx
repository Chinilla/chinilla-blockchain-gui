...
/*       An NFT offer consists of a single NFT being offered for HCX/CAT      */
/* ========================================================================== */
          ...
          <Box style={{ position: 'relative', top: '20px' }}>
            <TooltipIcon>
              <Trans>
                ...
                recommended minimum fee is 0.000005 HCX (5,000,000 vojos)
              </Trans>
            </TooltipIcon>
          </Box>
        </Flex>
    ...
    if (
      ...
      switch (tokenWalletInfo.walletType) {
        case WalletType.STANDARD_WALLET:
          balanceString = vojoToChinillaLocaleString(walletBalance.spendableBalance, locale);
          balance = vojoToChinilla(walletBalance.spendableBalance);
          break;
        case WalletType.CAT:
          balanceString = vojoToCATLocaleString(walletBalance.spendableBalance, locale);
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*           Currently only supports a single NFT <--> HCX/CAT offer          */
/* ========================================================================== */
...
function buildOfferRequest(params: NFTBuildOfferRequestParams) {
  ...
  const baseVojoAmount: BigNumber =
    tokenWalletInfo.walletType === WalletType.CAT ? catToVojo(tokenAmount) : chinillaToVojo(tokenAmount);
  const vojoAmount = exchangeType === NFTOfferExchangeType.NFTForToken ? baseVojoAmount : baseVojoAmount.negated();
