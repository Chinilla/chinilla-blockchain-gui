...
import { OfferSummaryRecord, OfferTradeRecord } from '@chinilla/api';
import { useCheckOfferValidityMutation, useGetNFTInfoQuery, useGetNFTWallets } from '@chinilla/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['hcx', 'thcx'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.CHINILLA: // fall-through
...
function NFTOfferMakerFee(props: NFTOfferMakerFeeProps) {
  ...
  return (
    <Flex flexDirection="row" alignItems="center" gap={1}>
      ...
      <Typography>
        <Plural value={makerFee} one="vojo" other="vojos" />
      </Typography>
      <TooltipIcon>
        {imported ? (
          <Trans>
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const hcxMakerFee = vojoToChinilla(makerFee);
