import { Wallet, WalletType, type CATToken } from '@chinilla/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@chinilla/api-react';
import { Select, useCurrencyCode } from '@chinilla/core';
...
type BuildAssetSelectorListParams = {
  ...
  chinillaCurrencyCode: string;
};

function buildAssetSelectorList(params: BuildAssetSelectorListParams): WalletOfferAssetSelection[] {
  const { wallets, catList, rows, otherRows, selectedWalletId, chinillaCurrencyCode } = params;
...
function OfferAssetSelector(props: OfferAssetSelectorProps) {
  ...
  const options: WalletOfferAssetSelection[] = useMemo(() => {
    ...
    return buildAssetSelectorList({
      ...
      otherRows,
      selectedWalletId,
      chinillaCurrencyCode: currencyCode,
    });
