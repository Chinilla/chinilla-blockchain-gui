...
import type { OfferSummaryAssetInfo, OfferSummaryRecord } from '@chinilla/api';
import { vojoToCAT, vojoToChinilla, vojoToCATLocaleString, vojoToChinillaLocaleString } from '@chinilla/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> HCX assumption
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return vojoToChinillaLocaleString(amount, locale);
  }
  if (walletType === WalletType.CAT) {
    return vojoToCATLocaleString(amount, locale);
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.CHINILLA]) {
    ...
    if (assetId) {
      ...
      if (amountInVojos) {
        const amountInTokens = assetType === OfferAsset.CHINILLA ? vojoToChinilla(amountInVojos) : vojoToCAT(amountInVojos);
