import { useGetWalletsQuery, useCheckOfferValidityMutation } from '@chinilla/api-react';
import { Flex, ButtonLoading, Link, Loading, useShowError } from '@chinilla/core';
...
export default function OfferBuilderViewer(props: OfferBuilderViewerProps) {
  ...
  async function handleSubmit(values: OfferBuilderData) {
    ...
    const feeAmount = fee?.[0]?.amount ?? '0'; // TODO convert to vojo here insted of in hook
        ...
        {error ? (
          ...
          <Alert severity="error">
            <Trans>
              ...
              <Link
                target="_blank"
                href="https://chinillalisp.com/docs/tutorials/offers_gui_tutorial/#taker-attempts-to-accept-an-invalid-offer"
              >
                here
              </Link>{' '}
              to learn more.
