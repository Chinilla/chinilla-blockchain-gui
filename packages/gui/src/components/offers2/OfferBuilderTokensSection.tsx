...
import { Flex, Loading, catToVojo, vojoToCATLocaleString } from '@chinilla/core';
import { Tokens } from '@chinilla/icons';
...
export default function OfferBuilderTokensSection(props: OfferBuilderTokensSectionProps) {
  ...
  const [amountWithRoyalties, royaltiesByAssetId] = useMemo(() => {
    ...
    tokens.forEach((token) => {
      tokenAmountsWithRoyalties[token.assetId] = catToVojo(token.amount ?? 0);
    });
        ...
        if (royaltyPayment) {
          ...
          royaltiesByAssetIdLocal[assetId].push({
            ...
            payment: {
              ...
              displayAmount: vojoToCATLocaleString(royaltyPayment.amount),
            },
          });
        }
      });
    ...
    Object.entries(tokenAmountsWithRoyalties).forEach(([assetId, amount]) => {
      amountsWithRoyalties[assetId] = vojoToCATLocaleString(amount);
    });
  ...
  return (
    <OfferBuilderSection
      ...
      subtitle={<Trans>Chinilla Asset Tokens (CATs) are tokens built on top of HCX</Trans>}
