...
export default function OfferBuilderHCXSection(props: OfferBuilderHCXSectionProps) {
  ...
  const [amountWithRoyalties, royaltyPayments] = useMemo(() => {
    ...
    let amountWithRoyaltiesLocal = chinillaToVojo(amount);
    const rows: Record<string, any>[] = [];
    Object.entries(allRoyalties).forEach(([nftId, royaltyPaymentsLocal]) => {
      const matchingPayment = royaltyPaymentsLocal?.find((payment) => payment.asset === 'hcx');
      if (matchingPayment) {
    ...
    return [vojoToChinillaLocaleString(amountWithRoyaltiesLocal), rows];
  }, [allRoyalties]);
