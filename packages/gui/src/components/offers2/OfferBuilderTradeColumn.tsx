...
import { Offering, Requesting } from '@chinilla/icons';
...
import OfferBuilderHCXSection from './OfferBuilderHCXSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showHCX = !readOnly || !!hcx.length;
  ...
  const mutedHCX = nfts.length || tokens.length;
  const mutedTokens = hcx.length || nfts.length;
  const mutedNFTs = hcx.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showHCX && <OfferBuilderHCXSection name={`${name}.hcx`} offering={offering} muted={mutedHCX} />}
