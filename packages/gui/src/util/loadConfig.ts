...
export function getConfigRootDir(net = 'vanillanet'): string {
  ...
  return 'CHINILLA_ROOT' in process.env ? untildify(process.env.CHINILLA_ROOT) : path.join(homedir, '.chinilla', net);
}
...
export default async function loadConfig(net?: string): Promise<{
  ...
  try {
    ...
    const daemonPort = get(config, 'ui.daemon_port', 43400);
