...
export function getUserDataDir(): string {
  const chinillaRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(chinillaRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const chinillaRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${chinillaRootUserDataPath}`);
  app.setPath('userData', chinillaRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(chinillaRootUserDataPath, 'Local Storage', 'leveldb');
