...
const log = initDebug('chinilla-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `chinilla_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.chinilla;
    if (!requiredNamespace) {
      throw new Error('Missing required chinilla namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['chinilla:testnet', 'chinilla:vanillanet'].includes(item));
    if (!chain) {
    ...
    const { fingerprints, vanillanet } = pair;
    const instance = vanillanet ? 'vanillanet' : 'testnet';
    const accounts = fingerprints.map((fingerprint) => `chinilla:${instance}:${fingerprint}`);
...
export async function processSessionRequest(
  ...
  try {
    ...
    const isVanillanet = instance === 'vanillanet';
    if (isVanillanet !== pair.vanillanet) {
